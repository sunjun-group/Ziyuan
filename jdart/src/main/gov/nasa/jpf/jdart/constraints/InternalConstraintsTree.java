...
public class InternalConstraintsTree {
	...
	private static final class Node {
		...
		public boolean isExhausted() {
			DecisionData dec = decisionData();
			if (dec == null)
				return (data != null && data.getClass() != DontKnowData.class); // Dont
																				// know
																				// is
																				// not
																				// exhausted,
		...
		public boolean hasDecisionData() {
			...
			return true;
			// throw new IllegalArgumentException("Querying non-decision node
			// (depth: "+ depth +
		...
		public ResultData result(PathResult result) {
			...
			// throw new IllegalStateException("Attempting to finish already
			// explored path (data = " + data.getClass().getName() + "!");
			return null;
		}
		...
		public DontKnowData dontKnow() {
			...
			if (data.getClass() != DontKnowData.class) {
				// System.err.println("Attempting to fail already explored
				// path!");
				return null;
			}
		...
		public UnsatisfiableData unsatisfiable() {
			...
			// throw new IllegalStateException("Attempting to fail already
			// explored path!");
			return null;
		}
	...
	private Node currentTarget = root; // This is the node the valuation
	...
	public BranchEffect decision(Instruction insn, int branchIdx, Expression<Boolean>[] decisions) {
		...
		if (current.isExhausted() && !replay) { // FALK: check how exhaustion is
												// computed, maybe replay check
												// is not necessary
			diverged = true;
			return BranchEffect.INCONCLUSIVE;
		}
	...
	public Valuation solve(int last_node, int branch) {
		...
		while (!node.isOpen() || flag) {
			...
			if (exh)
				dec.decrementUnexhausted();
			if (node.hasDecisionData() && node.decisionData().branchInsn.getInstructionIndex() == last_node) {
				flag = false;
			}
		}
		currentTarget = node;
	...
	public Valuation findNext() {
		...
		while ((currentTarget = backtrack(currentTarget, true)) != null) {
			...
			if (dec == null) {
				...
				for (; node.depth > 0; node = node.parent)
					temp[node.depth] = node.decisionData().branchInsn.getInstructionIndex();
				temp[node.depth] = node.decisionData().branchInsn.getInstructionIndex();
				...
				int key = path1.get(length - 1)[0];
				int value = path1.get(length - 1)[1];
				path2.add(new int[] { key, 1 - value });
