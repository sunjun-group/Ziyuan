...
public class JDartOnceClient {
	private static Logger log = LoggerFactory.getLogger(JDartOnceClient.class);
...
//		}
	...
	public static void main(String[] args) throws IOException {
		if (args.length == 10) {
			...
			new JDartOnceClient().run(JDartServerOnce.constructJDartParams(classpathStr, mainEntry, className, methodName, paramString, app, site, node, branch), port);
		}else{
			new JDartOnceClient().run(JDartServerOnce.constructJDartParams(), 8989);
		}
	}
