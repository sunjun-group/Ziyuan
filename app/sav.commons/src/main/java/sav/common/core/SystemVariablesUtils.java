...
import static sav.common.core.SystemVariables.SAV_JUNIT_JAR;
...
public class SystemVariablesUtils {
	...
	public static String updateSavJunitJarPath(AppJavaClassPath appClasspath) {
		String jarPath = appClasspath.getPreferences().get(SAV_JUNIT_JAR);
		if (jarPath == null) {
			jarPath = SavJunitRunner.extractToTemp().getAbsolutePath();
			appClasspath.getPreferences().put(SAV_JUNIT_JAR, jarPath);
		}
		return jarPath;
	}
