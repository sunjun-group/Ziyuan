...
import learntest.activelearning.core.progress.RandomTestProgressRunner;
...
public class TestcaseProgressHandler extends AbstractHandler implements IHandler {
	
	public TestcaseProgressHandler() {
	}
	
	@Override
	public Object execute(ExecutionEvent event) throws org.eclipse.core.commands.ExecutionException {
		Job job = new Job("Run Random Testcases For Progress") {
...
//		SAVTimer.enableExecutionTimeout = true;
	...
	private void run(IJavaProject project, AppJavaClassPath appClasspath, LearntestSettings learntestSettings, IProgressMonitor monitor, MethodInfo method) {
		...
		RandomTestProgressRunner randomtestprogressRunner = new RandomTestProgressRunner();
		try {

				randomtestprogressRunner.run(appClasspath, method, learntestSettings);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
