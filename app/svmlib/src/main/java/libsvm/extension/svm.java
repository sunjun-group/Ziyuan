...
class Solver {
	...
	void Solve(int l, QMatrix Q, double[] p_, byte[] y_,
		   double[] alpha_, double Cp, double Cn, double eps, SolutionInfo si, int shrinking, TimeOut isTimeOut)
				   throws SVMTimeOutException
	{
		this.l = l;
		this.Q = Q;
		QD = Q.get_QD();
		...
		// initialize alpha_status
		{
			...
			for(int i=0;i<l;i++){
				if(Thread.interrupted() || isTimeOut.isTrue()){
					isTimeOut.setFlag(true);
					throw new SVMTimeOutException();
				}
		...
		while(iter < max_iter)
		{
			...
			if(select_working_set(working_set, isTimeOut)!=0)
			{
	...
	int select_working_set(int[] working_set, TimeOut isTimeOut) throws SVMTimeOutException
	{
...
final class Solver_NU extends Solver
{
	...
	void Solve(int l, QMatrix Q, double[] p, byte[] y,
		   ...
		   SolutionInfo si, int shrinking, TimeOut isTimeOut) throws SVMTimeOutException
	{
		this.si = si;
		super.Solve(l,Q,p,y,alpha,Cp,Cn,eps,si,shrinking, isTimeOut);
	}
...
public class svm {
	...
	private static void solve_c_svc(svm_problem prob, svm_parameter param,
					...
					double Cp, double Cn, TimeOut isTimeOut) throws SVMTimeOutException
	{
		int l = prob.l;
		...
		s.Solve(l, new SVC_Q(prob,param,y), minus_ones, y,
			alpha, Cp, Cn, param.eps, si, param.shrinking, isTimeOut);
	...
	private static void solve_nu_svc(svm_problem prob, svm_parameter param,
					double[] alpha, Solver.SolutionInfo si, TimeOut isTimeOut) throws SVMTimeOutException
	{
		int i;
		int l = prob.l;
		double nu = param.nu;
		...
		s.Solve(l, new SVC_Q(prob,param,y), zeros, y,
			alpha, 1.0, 1.0, param.eps, si, param.shrinking, isTimeOut);
		double r = si.r;
					...
					double[] alpha, Solver.SolutionInfo si, TimeOut isTimeOut) throws SVMTimeOutException
	{
		...
		s.Solve(2*l, new SVR_Q(prob,param), linear_term, y,
			alpha2, param.C, param.C, param.eps, si, param.shrinking, isTimeOut);
					...
					double[] alpha, Solver.SolutionInfo si, TimeOut isTimeOut) throws SVMTimeOutException
	{
		...
		Solver_NU s = new Solver_NU();
		s.Solve(2*l, new SVR_Q(prob,param), linear_term, y,
			alpha2, C, C, param.eps, si, param.shrinking, isTimeOut);
		...
		double Cp, double Cn, TimeOut isTimeOut) throws SVMTimeOutException
	{
		...
		switch(param.svm_type)
		{
			case svm_parameter.C_SVC:
				solve_c_svc(prob,param,alpha,si,Cp,Cn, isTimeOut);
				break;
			case svm_parameter.NU_SVC:
				solve_nu_svc(prob,param,alpha,si, isTimeOut);
				break;
			case svm_parameter.ONE_CLASS:
				solve_one_class(prob,param,alpha,si, isTimeOut);
				break;
			case svm_parameter.EPSILON_SVR:
				solve_epsilon_svr(prob,param,alpha,si, isTimeOut);
				break;
			case svm_parameter.NU_SVR:
				solve_nu_svr(prob,param,alpha,si, isTimeOut);
				break;
		}
	...
	private static void sigmoid_train(int l, double[] dec_values, double[] labels, 
				  double[] probAB, TimeOut isTimeOut) throws SVMTimeOutException
	{
		double A, B;
	...
	private static void svm_binary_svc_probability(svm_problem prob, svm_parameter param, 
			double Cp, double Cn, double[] probAB, TimeOut isTimeOut) throws SVMTimeOutException
	{
		int i;
		int nr_fold = 5;
		...
		sigmoid_train(prob.l,dec_values,prob.y,probAB, isTimeOut);
	}

	// Return parameter of a Laplace distribution 
	private static double svm_svr_probability(svm_problem prob, svm_parameter param) throws SVMTimeOutException
	{
		int i;
		int nr_fold = 5;
	...
	// Interface functions
	//
	public static svm_model svm_train(svm_problem prob, svm_parameter param) throws SVMTimeOutException
	{
		TimeOut isTimeOut = new TimeOut(false);
		   ...
		   param.svm_type == svm_parameter.NU_SVR)
		{
			...
			decision_function f = svm_train_one(prob,param,0,0, isTimeOut);
			...
			for(i=0;i<nr_class;i++){
				...
				for(int j=i+1;j<nr_class;j++)
				{
					...
					if(param.probability == 1)
					{
						...
						svm_binary_svc_probability(sub_prob,param,weighted_C[i],weighted_C[j],probAB, isTimeOut);
						probA[p]=probAB[0];
						probB[p]=probAB[1];
					}

					f[p] = svm_train_one(sub_prob,param,weighted_C[i],weighted_C[j], isTimeOut);
					for(k=0;k<ci;k++)
	...
	public static void svm_cross_validation(svm_problem prob, svm_parameter param, int nr_fold, double[] target) throws SVMTimeOutException
	{
		int i;
