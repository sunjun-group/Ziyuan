...
public class ClasspathLabelProvider extends LabelProvider {
	...
	/**
	 * (see org.eclipse.jdt.internal.debug.ui.launcher.
	 * RuntimeClasspathEntryLabelProvider#getImage(Object))
	 */
	@Override
	public Image getImage(Object element) {
		if (element instanceof IClasspathEntry) {
			...
			case IClasspathEntry.CPE_LIBRARY:
				...
				boolean external = path.uptoSegment(
						workspaceLocation.segmentCount()).equals(
						workspaceLocation);
				boolean source = (entry.getSourceAttachmentPath() != null && !Path.EMPTY
						.equals(entry.getSourceAttachmentPath()));
				String key = null;
				if (external) {
					if (path != null) {
			...
			case IClasspathEntry.CPE_VARIABLE:
				return DebugUITools
						.getImage(IDebugUIConstants.IMG_OBJS_ENV_VAR);
			...
			case IClasspathEntry.CPE_CONTAINER:
				try {
					IClasspathContainer cpcontainer = JavaCore
							.getClasspathContainer(cpentry.getPath(),
									fJavaProject);
