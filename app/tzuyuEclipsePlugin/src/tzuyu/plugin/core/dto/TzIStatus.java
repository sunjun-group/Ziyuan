...
public class TzIStatus {
   ...
   * @param message
   */
  private TzIStatus(int code, int severity, String message) {
    fCode = code;
    fSeverity = severity;
    fMessage = message;
  }
  ...
  public IStatus getStatus() {
    ...
    return new Status(fSeverity, TzuyuPlugin.PLUGIN_ID, fCode, m, null);
  }

  public IStatus getStatus(Throwable t) {
    String m = MessageFormat.format(fMessage, new Integer(fCode));
    return new Status(fSeverity, TzuyuPlugin.PLUGIN_ID, fCode, m, t);
  }
  ...
  public static final TzIStatus NO_LOCAL_RANDOOPJAR_ERROR =
    new TzIStatus(1001, IStatus.ERROR,
        "Error while retreiving Randoop archive (code {0}).");

  public static final TzIStatus COMM_NO_FREE_PORT =
    new TzIStatus(5000, IStatus.ERROR,
        "Randoop plug-in could not find a free communication port. Check your firewall settings (code {0}).");
  
  public static final TzIStatus COMM_TERMINATED_SESSION =
    new TzIStatus(5001, IStatus.ERROR,
        "Randoop closed communications on port {1} unexpectadely. Was the process killed? (code {0}).");
  
  public static final TzIStatus COMM_MESSAGE_CLASS_NOT_FOUND =
    new TzIStatus(5002, IStatus.ERROR,
        "Randoop plug-in received a message from Randoop with an Object whose class is not in the plug-ins classpath (code {0}).");
  
  public static final TzIStatus GENERATED_FILE_NOT_IN_PROJECT =
    new TzIStatus(10000, IStatus.ERROR,
        "The JUnit test file {1} is not in the project for which it was generated (code {0}).");
  
  public static final TzIStatus JAVA_MODEL_EXCEPTION =
    new TzIStatus(10001, IStatus.ERROR,
  ...
  public static final TzIStatus PART_INIT_EXCEPTION =
    new TzIStatus(10001, IStatus.ERROR,
    "The view part could not be created or initialized. Try again or open the view by hand (code {0})");
  
  public static final TzIStatus RESOURCE_REFRESH_EXCEPTION =
    new TzIStatus(10002, IStatus.ERROR,
        "An error occured while trying to refresh a resource. Try running the operation again (code {0})");

  public static final TzIStatus NO_JAVA_PROJECT =
    new TzIStatus(20003, IStatus.ERROR,
  ...
  public static IStatus createUIStatus(int severity, String message) {
    return new Status(severity, TzuyuPlugin.PLUGIN_ID,
  ...
  public static IStatus createLaunchConfigurationStatus(int severity, String message, Throwable exception) {
    return new Status(severity, TzuyuPlugin.PLUGIN_ID, RANDOOP_LAUNCH_CONFIGURATION,
        message, exception);
  }
