...
import tzuyu.plugin.preferences.component.TableViewerEditablePanel;
...
public class TypeScopesTablePanel extends TableViewerEditablePanel<TypeScope> {
	...
	protected void onSelectTableRow(StructuredSelection selection) {
		...
		for (TypeScope scope : scopes) {
			if (Constants.CLASS_CLASS_NAME.equals(scope.getFullyQualifiedName()) ||
					Constants.OBJECT_CLASS_NAME.equals(scope.getFullyQualifiedName())) {
				size --;
			}
		}
	...
	protected void onRemove(List<TypeScope> elements) {
		...
		scopes.remove(scopeMap.get(Constants.CLASS_CLASS_NAME));
		scopes.remove(scopeMap.get(Constants.OBJECT_CLASS_NAME));
