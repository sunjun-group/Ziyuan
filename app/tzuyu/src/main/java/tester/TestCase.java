...
public class TestCase {
	 ...
	 * @param nodes
	 *            the parameter sequences that generate the concrete parameters
	 *            for the statements.
	 */
	...
	public TestCase extend(TestCaseNode newNode) {
		Query newQuery = abstractTrace.extend(newNode.getStatement()
				.getAction());
		...
		// for constructor, if the receiver in last statement is null, we need
		// set
		// the execution result of the current statement as the initial
		// receiver,
		// if the receiver in last statement is not null, we do nothing, then
		// the
	...
	public QueryTrace execute(IInstrumentor instrumentor) {
		...
		for (int index = 0; index < statements.size(); index++) {
			...
			for (int i = 0; i < indices.size(); i++) {
				...
				Variable var = new Variable(sequence, size + idx.stmtIdx,
						idx.argIdx);
				inputVars.add(var);
			}

			ExecutionResult result = RuntimeExecutor.executeGuard(action,
					inputVars);
			...
			if (result.isPassing()) {
				...
				if (normal) {
					// to filter the trace in which there are multiple
					// constructors
					...
					lastTrace = new QueryTrace(query, newSeq, states,
							TVAnswer.Rejecting, query.size() - 2);
					// for parameters that throws exception, we terminate the
					// process
					break;
				}
			} else {
				lastTrace = new QueryTrace(query, newSeq, states,
						TVAnswer.Unknown, query.size() - 2);
				// for parameters that cannot pass guard test, we terminate the
				// process
				break;
			}
		}
