...
public class TzuYuTesterV2 {
  ...
  /**
   * Test the the trace for the give query by generating multiple test cases for
   * it and then execute each test cases by collecting the runtime result and
   * return to the query handler. We don't add another constructor call for each
   * trace since in this version, the constructors are treated as normal
   * methods. For the epsilon alphabet we always return true.
   * 
   * @param query
   * @return
   */
  ...
  public boolean confirmWishfulThinking(TzuYuAction stmt) {
    if (cachedUnkownResult.contains(stmt)) {
      ...
      List<TestCase> cases = tcg.findFailedEvidenceForUnknownStatement(stmt);
      boolean found = false;
  ...
  /**
   * Execute all the generated test cases so far. Since the test cases does not 
   * save the instrumented states with it, every time if we want to get the 
   * instrumented states of the test case, we have to execute it.
   * @return
   */
  ...
  public List<Sequence> getAllTestCases() {
