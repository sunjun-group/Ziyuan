...
public class ParameterSelector {
  ...
  /*private List<Variable> selectParameters(Sequence seq, TzuYuAction stmt) {
    ...
    for (int index = 0; index < inputTypes.size(); index++) {
      ...
      // there are 3 sources of input variables
      int source = Randomness.nextRandomInt(3);
      Variable selectedVar = null;
      if (source == 0) {// select from the sequence
        List<Variable> vars = seq.getMatchedVariable(type);
        if (vars.size() != 0) {
          selectedVar = Randomness.randomMember(vars);
        }// there may be no matching variable in the cache
      } else if (source == 1) {
        selectedVar = this.selectCachedParameters(type);
      } else {
        selectedVar = this.generateNewVariableForType(type);
      }
      // Default case when there is no sequence in the cache,
      // then we have to generate a variable for this type.
      if (selectedVar == null) {
        selectedVar = this.generateNewVariableForType(type);
      }
...
/*  private Variable selectCachedParameters(Class<?> t) {
    ...
    Variable var = seqChosen.randomVarOfTypeLastStatement(t, m);
    return var;
  }
*/
  /**
    ...
    if (PrimitiveTypes.isBoxedOrPrimitiveOrStringOrEnumType(type)) {
      ...
      if (type.isArray()) {
        ...
        for (int j = 0; j < types.length; j++) {
          Variable inputVar = generateNewVariableForType(types[j].getType());
          seqs.add(inputVar.owner);
          inputVars.add(inputVar);
        }
        Sequence paramSeq = Sequence.concatenate(seqs);
 ...
 /* private InputAndSuccessFlag selectInputs(Variable receiver, TzuYuAction stmt) {
    // the input types contains the values required as
    // input to the given statement
    List<Class<?>> inputTypes = stmt.getInputTypes();
    // The list contains the sequences that generated the input variables
    // corresponding to the input types.
    List<Sequence> sequences = new ArrayList<Sequence>();
    // The variable indices for each generated input variable.
    List<VarIndex> variables = new ArrayList<VarIndex>();
    int totStatements = 0;
    ...
    for (int index = 0; index < inputTypes.size(); index++) {
      ...
      boolean isReceiver = (index == 0 
          && stmt.getAction() instanceof RMethod 
          && !((RMethod) stmt.getAction()).isStatic());
      Sequence chosenSeq = null;
      VarIndex chosenVar = null;
      if (isReceiver) {
        // For receiver we only use the main object modified by the
        // input main sequence can be used.
        chosenSeq = receiver.owner;
        chosenVar = new VarIndex(receiver.stmtIdx, receiver.argIdx);
      } else {
        // For non-receiver parameters there are 3 sources:
        int source = Randomness.nextRandomInt(3);
        Variable randomVar = null;
        if (source == 0) {// Source 1: select from the sequence
          List<Variable> vars = receiver.owner.getMatchedVariable(type);
          if (vars.size() != 0) {
            randomVar = Randomness.randomMember(vars);
          }// There may be no matching variable in the cache
        } else if (source == 1) { // Source 2: from cached sequences
          randomVar = this.selectCachedParameters(type);
        } else {// Source 3: Randomly generate new one
          randomVar = this.generateNewVariableForType(type);
        }
        // Default case when there is no sequence in the cache,
        // then we have to generate a variable for this type.
        if (randomVar == null) {
        ...
        // If there is still no matching variable, we abort, so
        // cannot get a variable for the specified input.
        if (randomVar == null) {
          return new InputAndSuccessFlag(false, sequences, variables);
        } else {
      ...
      // Update the statement index of the chosen variable
      chosenVar.stmtIdx += totStatements;

      variables.add(chosenVar);
...
*/
   ...
   * needs other parameters we need to generate them. If the constructor needs a
   * parameter of the same type as the <code>target</code>, we should take care
   * to handle the recursive parameter construction procedure by limiting the
   * depth of this recursive call.
   * 
   * @param target
   * @return
   */
  public InputAndSuccessFlag selectCtor(Class<?> target) {
    ...
    ConstructorInfo[] ctors = Analytics.getClassInfo(target).getConstructors();
    ...
    if (ctors.length == 0) {
      ...
      List<ClassInfo> subClasses = Analytics.getAccessibleClasses(target);
      if (subClasses.size() == 0) {
        throw new TzuYuException("no accessible constructor for class: "
            + target);
      } else {
   ...
   * @return
   */
  public InputAndSuccessFlag getInputsAfterFailed(
      Variable receiver, TzuYuAction statement, List<Integer> argIdx) {
