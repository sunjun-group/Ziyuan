...
public class TestCaseStore {
  ...
  public List<TestCase> findFailedEvidence(TzuYuAction statement) {
    ...
    for (Query query : keys) {
      ...
      if (lastAction.equals(statement)) {
        ...
        // the guard is different, we need to check whether the parameters 
  ...
  public Pair<List<TestCase>, List<TestCase>> getRepresentativeForAllTestCases() {
    ...
    Set<Query> normalKeys = normalTraces.keySet();
    for (Query query : normalKeys) {
      //Randomly select one not all the normal test cases in order to improve  
      //performance of testing.
      TestCase normalCase = selectANormalTrace(query);
      normalCases.add(normalCase);
    }
    
    Set<Query> errorKeys = errorTraces.keySet();
    for (Query query : errorKeys) {
      //Randomly select one not all the error test cases in order to improve
      //performance of testing
      TestCase errorCase = selectAnErroneousTrace(query);
      errorCases.add(errorCase);
    }
    
    return new Pair<List<TestCase>, List<TestCase>>(normalCases, errorCases);
  }
