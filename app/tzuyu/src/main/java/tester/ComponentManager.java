...
public class ComponentManager {
  ...
  public SimpleList<Sequence> getSequencesForType(StatementKind statement, 
    ...
    SimpleList<Sequence> ret = 
        gralComponents.getSequencesForType(neededType, false);
    ...
    if (classLiterals != null || packageLiterals != null) {
      ...
      if (statement instanceof RMethod) {
        ...
        declaringCls = 
            ((RConstructor) statement).getConstructor().getDeclaringClass();
      }

      if (classLiterals != null) {
        if (declaringCls != null) {
          SimpleList<Sequence> sl = 
              classLiterals.getSequences(declaringCls, neededType);
          if (!sl.isEmpty()) {
      ...
      if (packageLiterals != null) {
        ...
        if (pkg != null) {
          SimpleList<Sequence> sl = 
              packageLiterals.getSequences(pkg, neededType);
          if (!sl.isEmpty()) {
            ret = new ListOfLists<Sequence>(ret, sl);
          }
        }
