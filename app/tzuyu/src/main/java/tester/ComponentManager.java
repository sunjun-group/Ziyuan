...
import tzuyu.engine.store.ClassLiterals;
import tzuyu.engine.store.PackageLiterals;
import tzuyu.engine.store.SequenceCollection;
...
public class ComponentManager {
	...
	public void config(TzConfiguration configuration) {
		gralComponents.config(configuration);
	}
	...
	public SimpleList<Sequence> getSequencesForType(StatementKind statement,
		...
		if (classLiterals != null || packageLiterals != null) {
			...
			if (statement instanceof RMethod) {
				declaringCls = ((RMethod) statement).getMethod()
						.getDeclaringClass();
			} else if (statement instanceof RConstructor) {
				declaringCls = ((RConstructor) statement).getConstructor()
						.getDeclaringClass();
			}

			if (classLiterals != null) {
				if (declaringCls != null) {
					SimpleList<Sequence> sl = classLiterals.getSequences(
							declaringCls, neededType);
					if (!sl.isEmpty()) {
			...
			if (packageLiterals != null) {
				...
				if (pkg != null) {
					SimpleList<Sequence> sl = packageLiterals.getSequences(pkg,
							neededType);
					if (!sl.isEmpty()) {
						ret = new ListOfLists<Sequence>(ret, sl);
					}
