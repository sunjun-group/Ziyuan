...
/*  *//**
   * Test the membership query, the testing strategy for membership test is that
   * we use the largest normal prefix and then generate random parameters for
   * the remaining statements. The query result contains the query traces
   * correspond to the maximum normal prefix of the original query.
   * 
   * @param query
   * @return
   *//*
  private QueryResult memberTest2(Query query) {
    // Special handling of the epsilon query
    if (query.isEpsilon()) {
      ...
      if (input.success) {
        ...
        if (runtime.isSuccessful()) {
          QueryTrace trace = new QueryTrace(query, seq,
              new ArrayList<Prestate>(), TVAnswer.Accepting, -1);

          traceStore.addNormal(query, trace);

          QueryResult result = new QueryResult();
          result.positiveSet.add(trace);

          return result;
        } else {
          // The parameter sequence failed due to some reason,
          // here we return a failed trace, since the found parameters
          // makes the trace fail.
          QueryTrace trace = new QueryTrace(query, seq,
    ...
    // Find the longest prefix already executed
    QueryTrace trace = findLargestPrefixTrace(query);
    // There is not good sequence for this query or the query
    // is confirmed to have a bad sequence, then we know the
    // answer is rejecting.
    if (trace.isRejected()) {
    ...
    // for multiple tests.
    Query remainingQuery = query.getRemainingQuery(prefix);

    int tracesPerQuery = Options.tracesPerQuery();

    Query currentQuery = prefix;
    for (int index = 0; index < remainingQuery.size(); index++) {
      ...
      // ensure the statement will be executed one
      boolean hasArgument = true;

      for (int count = 0; count < tracesPerQuery && hasArgument; count++) {
        // Update the hasArgument flag
        hasArgument = !noArgument;
        // Select inputs for the parameters
        InputAndSuccessFlag inputs = selector.selectInputs(receiver, stmt);
        // Prepare input variables for guard condition evaluation
        ...
        if (inputs.success) {
          ...
          for (int i = 0; i < vars.size(); i++) {
            ...
            Variable var = new Variable(sequence, idx.stmtIdx, idx.argIdx);
            inputVars.add(var);
          }
          // Evaluate the guard by generating runtime values for parameters
          ExecutionResult result = RuntimeExecutor
              .executeGuard(stmt, inputVars);
          ...
          if (!result.isPassing()) {
            ...
            QueryTrace newTrace = new QueryTrace(newQuery, newSeq, states,
                TVAnswer.Unknown, newQuery.size() - 2);
            traceStore.addUnknown(newQuery, newTrace);
            // Try to find old values used and execute it to pass the guard
            // condition
            if (hasArgument) {
            ...
            // Instrument to get the pre-state.
            Prestate prestate = Prestate.log(inputVars, result.getRuntime());
            // Merge the pre-states
            List<Prestate> states = new ArrayList<Prestate>(
                currentTrace.getStates());
            states.add(prestate);
            ...
            Sequence newSeq = sequence.extend(stmt, inputVars);
            newSeq.updateReceiver();
            // Execute the statement to see whether is OK or throw exceptions
            boolean normal = RuntimeExecutor.executeStatement(stmt,
                result.getRuntime());

            if (normal) {
              QueryTrace newTrace = new QueryTrace(newQuery, newSeq, states,
                  TVAnswer.Accepting, newQuery.size() - 1);
              traceStore.addNormal(newQuery, newTrace);
            } else {
              QueryTrace newTrace = new QueryTrace(newQuery, newSeq, states,
                  TVAnswer.Rejecting, newQuery.size() - 2);
              traceStore.addError(newQuery, newTrace);
            }
          }
        } else {
          // TODO how about we cannot select inputs for the parameters
          throw new TzuYuException("Cannot select inputs for the parameters");
        }
      }
      ...
      if (!queryResult.isAccepting()) {
        // Failed in the intermediate statements,
        // We return this successfully executed prefix and the first failed
        // statement
        return queryResult;
      } else {// All sequences are normal execution
...
*/
  /**
   * We test the membership query starts from the beginning to the end. But this
   * cause the problem of non convergence problem. Since for some query the same
   * action may fail in one trace and it may succeed in another trace.
   * 
   * @param query
   * @return
   */
  public QueryResult memberTest(Query query) {
    ...
    if (input.success) {
      Variable initReceiver = new Variable(ctorSeq, ctorSeq.size() - 1, -1);
    ...
    for (int index = 0; index < query.size(); index++) {
      ...
      if (!currentTrace.isAccepted()) {
        List<QueryTrace> errorTraces = traceStore.findError(currentQuery);
        QueryResult result = new QueryResult();
        result.negativeSet.addAll(errorTraces);
        return result;
      }
      ...
      boolean noArgument = stmt.getAction().hasNoArguments();
      // We need to execute the query at least once so, initialize it to true
      boolean hasArgument = true;
      ...
      for (int count = 0; count < tracesPerQuery && hasArgument; count++) {
        ...
        // more test inputs
        InputAndSuccessFlag inputs = selector.selectNewInputs(receiver, stmt);
        ...
        if (inputs.success) {
          ...
          // Evaluate the guard by generating runtime values for parameters
          ExecutionResult rslt = RuntimeExecutor.executeGuard(stmt, inputVars);

          if (rslt.isPassing()) {
            Prestate prestate = Prestate.log(inputVars, rslt.getRuntime());
            ...
            // The failed execution may due to the unsuccessfully generation
            // of the input variables, in this case we cannot instrument the
            // Pre-state.
            Prestate prestate = Prestate.log(inputVars, rslt.getRuntime());
            List<Prestate> states = new ArrayList<Prestate>(
                currentTrace.getStates());
            ...
            traceStore.addUnknown(newQuery, newTrace);
            // If the guard test failed, we ignore the input and try to
            // generate another parameters and execute it to pass the condition
            if (hasArgument) {
              testAfterAllFailed(currentTrace, receiver, stmt);
            }
          }
  ...
  /*
  private QueryTrace findLargestPrefixTrace(Query query) {
    Query currentQuery = query;
    Query prefix = currentQuery.getImmediatePrefix();
    while (prefix.size() >= 0) {
      QueryTrace trace = traceStore.findNormalRandomly(prefix);
      if (trace != null) {
        return trace;
      }
      // Try to find the bad sequence for the prefix
      trace = traceStore.findErrorRandomly(prefix);
      if (trace != null) {
   ...
   * equivalence query. The testing strategy is that we don't reuse any existing
   * prefix such that we can discover more test cases. The generated query
   * result contains only the maximum successfully executed prefix until a
   * failure occurred.
   * 
   * @param query
  ...
  public QueryResult candidateTest(Query query) {
    ...
    for (int index = 0; index < query.size(); index++) {
      ...
      if (!currentTrace.isAccepted()) {
        List<QueryTrace> errorTraces = traceStore.findError(currentTrace.query);
   ...
   * @param stmt
   */
  public void testAfterAllFailed(
      QueryTrace currentTrace, Variable receiver, TzuYuAction stmt) {
    ...
    while (permutation.hasNext()) {
      ...
      InputAndSuccessFlag inputs = selector.getInputsAfterFailed(receiver,
          stmt, argIndex);
      // Prepare input variables
      List<Variable> inputVars = new ArrayList<Variable>();
      if (inputs.success) {
        ...
        if (rslt.isPassing()) {
          ...
          boolean normal = 
              RuntimeExecutor.executeStatement(stmt, rslt.getRuntime());
          ...
          if (normal) {
            ...
            traceStore.addNormal(newQuery, newTrace);
            // Find one and then exit, don't need to do other testings
            return;
          } else {
            QueryTrace newTrace = new QueryTrace(newQuery, newSeq, states,
                TVAnswer.Rejecting, newQuery.size() - 2);
  ...
  public boolean confirmWishfulThinking(QueryTrace queryTrace) {
    ...
    if (cachedUnkownResult.contains(stmt)) {
      ...
      boolean found = traceStore.findFailedEvidenceForUnknownTransition(stmt);
      if (found) {
        cachedUnkownResult.add(stmt);
      }

      // If we cannot find such a transition then we return true, which means
      // the transition is OK according to our wishful thinking; If such a
      // transition exists in the error traces, we must return false, which
