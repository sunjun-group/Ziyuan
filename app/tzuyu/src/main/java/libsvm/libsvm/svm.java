...
class Cache {
  ...
  Cache(int l_, long size_) {
    ...
    size = Math.max(size, 2 * (long) l); // cache must be large enough for two
                                         // columns
    lru_head = new head_t();
...
final class Solver_NU extends Solver {
  ...
  void Solve(int l, QMatrix Q, double[] p, byte[] y, double[] alpha, double Cp,
...
public class svm {
  ...
  private static void solve_nu_svc(svm_problem prob, svm_parameter param,
    ...
    s.Solve(l, new SVC_Q(prob, param, y), zeros, y, alpha, 1.0, 1.0, param.eps,
        si, param.shrinking);
    double r = si.r;
  ...
  public static svm_model svm_train(svm_problem prob, svm_parameter param) {
    ...
    if (param.svm_type == svm_parameter.ONE_CLASS
      ...
      decision_function[] f = new decision_function[nr_class * (nr_class - 1)
          / 2];
