...
public class svm_toy extends Applet {
  ...
  /**
	 * 
	 */
  private static final long serialVersionUID = -589761789187426292L;
  ...
  final static Color colors[] = { new Color(0, 0, 0), new Color(0, 120, 120),
      new Color(120, 120, 0), new Color(120, 0, 120), new Color(0, 200, 200),
  ...
  void button_save_clicked(String args) {
    ...
    try {
      ...
      if (svm_type_idx != -1) {
        StringTokenizer svm_str_st = new StringTokenizer(args.substring(
            svm_type_idx + 2).trim());
  ...
  protected void processMouseEvent(MouseEvent e) {
    if (e.getID() == MouseEvent.MOUSE_PRESSED) {
      if (e.getX() >= XLEN || e.getY() >= YLEN)
        return;
      point p = new point((double) e.getX() / XLEN, (double) e.getY() / YLEN,
          current_value);
