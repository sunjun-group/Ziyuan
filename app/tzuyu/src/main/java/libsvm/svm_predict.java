...
class svm_predict {
  ...
  private static void exit_with_help() {
    System.err.print("usage: svm_predict [options] test_file model_file " 
            + "output_file\n options:\n"
            + "-b probability_estimates: whether to predict probability " 
            + "estimates, 0 or 1 (default 0); one-class SVM not supported" 
            + " yet\n");
    System.exit(1);
  }
  ...
  public static void main(String argv[]) throws IOException {
    ...
    try {
      ...
      if (predict_probability == 1) {
        ...
        if (svm.svm_check_probability_model(model) != 0) {
          System.out.print("Model supports probability estimates," 
