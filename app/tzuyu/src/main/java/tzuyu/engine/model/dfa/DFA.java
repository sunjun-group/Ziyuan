...
public class DFA {
  ...
  /**
   * add the state into the states of the DFA. if the state to be added already
   * exists ( its id equals to the id of some state), just ignore it
   * 
   * @param state
  ...
  public void print() {
    ...
    System.out.println("initial state:<" + states.get(initStateID).id + ">");
    ...
    for (State state : states) {
      for (Transition tran : state.trans) {
        System.out.println("(" + state.id + "--" + tran.action.toString()
  ...
  public TracesPair randomWalk(int traceSize, int traceLength) {
    ...
    for (int i = 0; i < traceSize; i++) {
      ...
      for (int length = 0; length < traceLength; length++) {
        ...
        Transition transition = currentState.trans.get(selectedTransitionID);
  ...
  public TracesPair randomWalk2(int traceSize, int maxTraceLength) {
    ...
    for (int i = 0; i < traceSize; i++) {
      ...
      for (int length = 0; length < maxTraceLength; length++) {
        //For the first action, we only need to execute one constructor.
        //For the remaining actions, we are not allowed to execute constructors
        Transition transition = selectTransition(currentStateId, length == 0);
        currentStateId = transition.target;
        // Here we just want to append the action to the tail of str.
        // For performance reason DO NOT CALL concatAtTail();
        str.appendAtTail(transition.action);
  ...
  /**
   * Select randomly an outgoing transition in which the action is either 
   * a constructor or a normal method.
   * @param stateID
   ...
   *         <code>len</code> prefix of the input string is accepting, then the
   *         last transition is the last transition in the prefix and the number
   *         of traversed transitions is <code>len</code>. The return value lies
   *         in <code>[0, len]</code>.
   */
  ...
  public String createDotRepresentation() {
    ...
    for (State state : states) {
      for (Transition tran : state.trans) {
        result.append("\t" + state.id + " -> " + states.get(tran.target).id);
        result.append(" [label=\"");
