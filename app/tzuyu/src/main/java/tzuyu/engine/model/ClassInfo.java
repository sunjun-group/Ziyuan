...
public abstract class ClassInfo {
  ...
  public abstract ObjectInfo cloneMockup(Object object, int level);
  ...
  /**
   * Get all the fields defined in the specified level. The level starts from 0.
   * The fields defined on level 0 are the reference fields corresponding to the
   * parameter types themselves. Fields defined on level 1 is the fields
   * declared directly on the parameter classes.
   * 
   * @param level
   * @return
   */
  ...
  public final ObjectInfo clone(Object object) {
    return cloneMockup(object, 0);
  }
  ...
  public MethodInfo[] getMethods() {
    ...
    if (!Options.useInheritedMethods()) {
      return localMethods;
    }
  ...
  /**
   * Filter the methods in the super classes that are overridden by the methods
   * in the subclass. We consider the methods to be overridden by considering 
   * the signature and return type and the accessors (only public or default 
   * can be overridden).
   * @param subs
   * @param supers
   * @return
  ...
  private boolean isOverridden(MethodInfo subMethod, MethodInfo superMethod) {
    ...
    if (subMethod.getName().equals(superMethod.getName())) {
      ...
      if (Arrays.equals(subMethod.getParameterTypes(),
        ...
        if (subMethod.getReturnType().equals(superMethod.getReturnType())) {
          return true;
        }
      }
    } 
