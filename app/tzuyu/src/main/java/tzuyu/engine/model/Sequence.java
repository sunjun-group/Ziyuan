...
public class Sequence implements Serializable {
  ...
  public void setReceiver(Variable newReceiver) {
    ...
    if (!this.equals(newReceiver.owner)) {
      throw new TzuYuException("the new receiver's owner is not this sequence");
    }
  ...
  public Variable getReceiver() {
    if (receiver == null) {
      throw new TzuYuException("receiver is not set before get");
    }
  ...
  private void computeLastStatementInfo() {
    ...
    if (this.statements.size() > 0) {
      ...
      returnVar.add(new Variable(this, statements.size() - 1, -1));
      ...
      if (inputVars.size() != inputTypes.size()) {
        throw new TzuYuException("number of parameters does not match");
      }
   ...
   * <code>stmtIndex</code> of each variable is the absolute statement index of
   * this sequence. NOTE THAT: this operation returns a new sequence which means
   * it will not affect this sequence.
   * 
   * @param stmt
   *          statement to be appended to the end of this sequence.
   * @param inputVars
   *          the input variables for the <code>stmt</code>.
   * @return the newly created sequence that is equal to <code>this+stmt</code>.
   */
  public Sequence extend(TzuYuAction stmt, List<Variable> inputVars) {
    List<RelativeNegativeIndex> indexList = 
        new ArrayList<RelativeNegativeIndex>(1);
    ...
    return new Sequence(new OneMoreElementList<Statement>(statements, newStmt),
        savedHashCode + newStmt.hashCode(), netSize);
  }
  
  public Sequence extend(Statement stmt) {
    int netSize = stmt.isPrimitive() ? savedNetSize : (savedNetSize + 1);
    return new Sequence(new OneMoreElementList<Statement>(statements, stmt), 
        savedHashCode + stmt.hashCode(), netSize);
  }
  
  /**
   * Calculate the relative negative index for the given variable. The relative
   * negative index is calculated by -(<code>stmtIndex - v.stmtIndex</code>).
   * 
   * @param stmtIndex
   * @param v
   * @return
  ...
  /**
   * The hash code of a sequence is the sum of every statement's hash code. This
   * seems good enough and it makes computing hash code of a concatenation of
   * sequences faster (it's just the addition of each sequences's hash code).
   * Otherwise, hash code computation used to be a hot spot.
   * 
   * @param statements
   * @return
   */
  ...
  public List<Variable> getMatchedVariable(Class<?> type) {
    ...
    for (int index = 0; index < statements.size(); index++) {
      ...
      if (varTypeMatches(stmt.getOutputType(), type, Match.COMPATIBLE_TYPE)) {
  ...
  public void updateReceiver() {
    ...
    if (lastStatement.statement.getAction().hasReceiverParameter()) {
      Variable newReceiver = new Variable(this, size() - 1, 0);
      this.receiver = newReceiver;
    }
  }
  ...
  /**
   * Get the variable sequence for the parameter at index <code>varIndex</code>
   * of the statement corresponding to the query action at index
   * <code>reverseQueryIndex</code> in reverse order. The variable sequence is
   * an independent sequence that generated the parameter value in the statement
   * at index <code>reverseStmtIndex</code> in reverse order.
   * 
  ...
  public Variable getVariableSequence(int reverseQueryIndex, int varIndex) {
    ...
    for (int index = 0; index < reverseQueryIndex; index++) {
      ...
      RelativeNegativeIndex relativeIndex = statement.getInputVars().get(0);
      stmtIndex = stmtIndex + relativeIndex.stmtIdx;
    }

    Statement stmt = statements.get(stmtIndex);
    // For the first parameter which could be the receiver for instance method
    ...
    if (varIndex == 0) {
      ...
      RelativeNegativeIndex preVarIdx = stmt.getInputVars().get(varIndex - 1);
      RelativeNegativeIndex relVarIdx = stmt.getInputVars().get(varIndex);
  ...
  public void printStatement(StringBuilder b, int index) {
    ...
    // Example: { "var2", "(int)3" }
    statements.get(index).appendCode(getVariable(index), getInputs(index), b);
  }
