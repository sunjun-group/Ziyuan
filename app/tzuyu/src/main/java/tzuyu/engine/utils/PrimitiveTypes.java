...
public final class PrimitiveTypes {
  ...
  public static final Map<String, Class<?>> typeNameToPrimitiveOrString = 
      new LinkedHashMap<String, Class<?>>();
  static {
  ...
  public static boolean isBoxedOrPrimitiveOrStringOrEnumType(Class<?> c) {
    if (isPrimitive(c)) {
      return true;
    }
    ...
    if (c.isEnum()) {
      // We treat enumeration type as primitive
      return true;
    }
  ...
  public static String toCodeString(Object value) {
    ...
    if (String.class.equals(valueClass)) {
      ...
      if (escaped.length() > Options.stringMaxLen()) {
      ...
      return "\'"
          + StringEscapeUtils.escapeJavaStyleString(value.toString(), true)
          + "\'";
  ...
  public static boolean stringLengthOK(String s) {
    ...
    if (length > Options.stringMaxLen()) {
    ...
    // right away.
    // If a string's length is less than 1/6 * maxlen, it's definitely
    // short enough, since
    ...
    if (length * 6 < Options.stringMaxLen()) {
      stringLengthOKCached.put(s, true);
      return true;
    }

    boolean retval = StringEscapeUtils.escapeJava(s).length() <= Options
        .stringMaxLen();
