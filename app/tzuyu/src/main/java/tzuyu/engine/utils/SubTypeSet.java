 ...
 * {@link ReflectionUtils.util.Reflection#canBeUsedAs(Class,Class)})
 * relationships.
 */
public class SubTypeSet {
  ...
  public void add(Class<?> c) {
    ...
    for (Class<?> cls : subTypesWithsequences.keySet()) {
      if (ReflectionUtils.canBeUsedAs(c, cls)) {
  ...
  private void addQueryType(Class<?> c) {
    ...
    for (Class<?> classWithsequence : typesWithsequences.getElements()) {
      if (ReflectionUtils.canBeUsedAs(classWithsequence, c)) {
   ...
   */
  public
      boolean containsAssignableType(Class<?> c, ReflectionUtils.Match match) {
    if (!subTypesWithsequences.keySet().contains(c)) {
      addQueryType(c);
    }
    ...
    if (match == ReflectionUtils.Match.COMPATIBLE_TYPE) {
