...
public class SequencePrettyPrinter {
  ...
  public String prettyPrintSequences () {
    ...
    sb.append(Globals.lineSep);
    sb.append(indent(className + " " +mainObj + " = new " + className + "();", 4));
    sb.append(Globals.lineSep);
  ...
  public File createFile(String output_dir) {
    ...
    File outputFile = new File(output_dir + 
        System.getProperty("file.separator") + this.className + ".java");
    ...
    for (Sequence sequence : this.outputSequences) {
      ...
      for (int i = 0; i < length; i++) {
        ...
        if (needImport(outputType)) {
          import_clz_set.add(ReflectionUtils.getCompilableName(outputType));
        }
        //add input types
        for (Class<?> inputType : statement.getInputTypes()) {
          inputType = this.getComponentType(inputType);
          if (needImport(inputType)) {
            import_clz_set.add(ReflectionUtils.getCompilableName(inputType));
          }
        }
        ...
        if ((statement.getAction().getAction()) instanceof RMethod) {
          ...
          if (rmethod.isStatic()) {
            ...
            if (needImport(declaring_class)) {
              import_clz_set.add(ReflectionUtils.getCompilableName(declaring_class));
            }
          }
        }
      }
