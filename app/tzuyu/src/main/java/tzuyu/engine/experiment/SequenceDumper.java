...
class SequenceDumper {
   ...
   **********************************************************/
  private void appendCode(StringBuilder sb, Statement stmt,
  ...
  private void printRMethod(RMethod rmethod, Variable newVar, 
    ...
    if (!rmethod.isVoid()) {
      ...
      String cast = "";
      sb.append(" " + renamer.getRenamedVar(newVar.stmtIdx, newVar.argIdx) 
           + " = " + cast);
    }

    String receiverString = rmethod.isStatic() ? null : 
      renamer.getRenamedVar(inputVars.get(0).stmtIdx, inputVars.get(0).argIdx);
    ...
    for (int i = startIndex ; i < inputVars.size() ; i++) {
      ...
      if (PrimitiveTypes.isPrimitive(type) && Options.longFormat()) {
      ...
      if (!Options.longFormat() &&  stmt instanceof RAssignment) {
        sb.append(PrimitiveTypes.toCodeString(((RAssignment) stmt).getValue()));
      } else {
        sb.append(renamer.getRenamedVar(var.stmtIdx, var.argIdx));
      }
    }
  ...
  private void printRConstructor(RConstructor ctor, Variable newVar, 
      ...
      Class<?> declaringClass =
          ctor.getConstructor().getDeclaringClass();
      ...
      sb.append(declaringStr + " " 
          + renamer.getRenamedVar(newVar.stmtIdx, newVar.argIdx) + " = "
          + (isNonStaticMember ? renamer.getRenamedVar(inputVars.get(0).stmtIdx,
              inputVars.get(0).argIdx) + "." 
              : ""
            )
          + "new "
          + (isNonStaticMember ? declaringClass.getSimpleName() : declaringStr)
          + "(");
      for (int i = (isNonStaticMember ? 1 : 0) ; i < inputVars.size() ; i++) {
        ...
        if (!Options.longFormat() &&  sk instanceof RAssignment) {
          sb.append(PrimitiveTypes.toCodeString(((RAssignment) sk).getValue()));
        } else {
          sb.append(renamer.getRenamedVar(var.stmtIdx, var.argIdx));
        }
      }
  ...
  private void printPrimitiveType(RAssignment statement,
    ...
    //print primitive type
      if (type.isPrimitive()) {
        ...
        sb.append(" ");
        sb.append(this.renamer.getRenamedVar(newVar.stmtIdx, newVar.argIdx));
        sb.append(" = ");
        sb.append(PrimitiveTypes.toCodeString(statement.getValue()));
        sb.append(";");
        sb.append(Globals.lineSep);
