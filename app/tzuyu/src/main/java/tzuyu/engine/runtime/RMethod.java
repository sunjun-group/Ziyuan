...
public class RMethod extends StatementKind implements Serializable {
  ...
  public ExecutionOutcome execute(Object[] inputVals, PrintStream out) {
    ...
    if (thrown == null)
      return new NormalExecution(code.getReturnVariable(), inputVals, time);
    else
  ...
  public void appendCode(Variable newVar, List<Variable> inputVars, 
      StringBuilder b) {
    if (!isVoid()) {
      b.append(ReflectionUtils.getCompilableName(this.method.getReturnType()));
      String cast = "";
    ...
    for (int i = startIndex ; i < inputVars.size() ; i++) {
      ...
      if (PrimitiveTypes.isPrimitive(getInputTypes().get(i)) && Options.longFormat()) {
      ...
      if (!Options.longFormat()&& Sequence.canUseShortFormat(stmt)) {
        Object val = ((RAssignment) stmt.getAction().getAction()).getValue();
  ...
  public String getTypeArguments() {
    ...
    if (CollectionsExt.findAll(Arrays.asList(params), Object.class).size() 
        == params.length) {
      return "";
    }
    return b.toString();
