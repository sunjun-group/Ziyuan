...
public class RConstructor extends StatementKind implements Serializable {
  ...
  public List<Class<?>> getInputTypes() {
    if (inputTypesCached == null) {
      inputTypesCached = new ArrayList<Class<?>>(Arrays.asList(constructor
          .getParameterTypes()));
    }
  ...
  public void appendCode(Variable newVar, List<Variable> inputVars, 
    ...
    b.append(declaringStr + " " + newVar.getName() + " = "
    ...
    for (int i = (isNonStaticMember ? 1 : 0) ; i < inputVars.size() ; i++) {
      ...
      if (!Options.longFormat() && Sequence.canUseShortFormat(stmt)) {
        b.append(PrimitiveTypes.toCodeString(
            ((RAssignment) stmt.getAction().getAction()).getValue()));
      } else {
