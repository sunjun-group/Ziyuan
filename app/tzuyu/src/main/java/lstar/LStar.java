...
public class LStar {
   ...
   */
  private void updateWhenInconsistent(Trace conflict) {
  ...
  /**
   * The observation is not closed (open) then update according to the algorithm
   * defined in the paper
   * 
   * @param conflict
   *          the string whose row is not included in the S row.
   */
  private void updateWhenOpen(Trace conflict) {
  ...
  /**
   * The condition under which the observation table is consistent is that: for
   * all s1 and s2 in S, if row(s1) == row(s2) then for all action a in the
   * alphabet such that row(s1.a) == row(s2.a)
   * 
   * @return
   */
  private Trace isConsistent() {
    ...
    for (Trace key1 : keys) {
      for (Trace key2 : keys) {
        if (!key1.equals(key2)
            && otable.sRows.get(key1).equals(otable.sRows.get(key2))) {
          for (int i = 1; i < sigma.getSize(); i++) {
            Trace newKey1 = key1.concatenateAtTail(sigma.getAction(i));
            Trace newKey2 = key2.concatenateAtTail(sigma.getAction(i));
            if (!otable.getRow(newKey1).equals(otable.getRow(newKey2))) {
              Trace conflict = null;
              // save the a, key1 and key2 in the cache,
              // they will be used very soon
              for (int j = 0; j < otable.getRow(newKey1).length(); j++) {
                if (otable.getRow(newKey1).charAt(j) != otable.getRow(newKey2)
                    .charAt(j)) {
                  // save j;
                  conflict = otable.columns.get(j).concatenateAtFront(
                      sigma.getAction(i));
                  break;
                }
              }
   ...
   * every s1.a in RA row there exists a s2 in S such that row(s2) == row(s1.a)
   * 
   * @return
   */
   ...
   * @param cea
   */
  private void refineWithCounterExample(Trace cea) {
    ...
    for (Trace str : prefix) {
      if (!otable.sRows.containsKey(str)) {
        ...
        for (int i = 1; i < sigma.getSize(); i++) {
          ...
          for (int j = 0; j < otable.columns.size(); j++) {
            Trace newMemberStr = 
                newStr.concatenateAtTail(otable.columns.get(j));
