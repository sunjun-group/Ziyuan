...
public class TzuYuRefinerV2 {
   ...
   * transitions and generate the divider which can distinguish the inconsistent
   * transition. In dealing with the counterexamples traces, we prefer alphabet 
   * refinement to control refinement, since control refinement by L* may waste 
   * efforts to construct a DFA which would be destroyed later in case we find 
   * a divider which leads to a refined alphabet, since TzuYu will restart the 
   * whole learning process when the alphabet is refined. 
   * 
   * @param dfa
   ...
   *          
   * @return 
   *          the witness which contains the divider for the inconsistent 
  ...
  public Witness refineCandidate(DFA dfa, List<QueryTrace> traces) {
    ...
    Pair<Transition, QueryResult> result = 
        findInconsistentTransition(dfa, resultMap);
