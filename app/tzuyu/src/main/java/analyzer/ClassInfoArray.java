...
public class ClassInfoArray extends ClassInfo {
	...
	public ObjectInfo cloneMockup(Object object, int level, TzConfiguration config) {
		...
		if (object == null) {
			...
			;
			ObjectInfo element = baseType.cloneMockup(null, level + 1, config);
			if (element != null) {
		...
		if (!ReflectionUtils.canBeUsedAs(object.getClass(), type)) {
			...
			length = Math.min(length, config.getArrayMaxLength());
			List<ObjectInfo> elements = new ArrayList<ObjectInfo>();
			for (int index = 0; index < length; index++) {
				Object elementObject = Array.get(object, index);
				ObjectInfo element = baseType.cloneMockup(elementObject,
						level + 1, config);
				if (element != null) {
					elements.add(element);
				}
	...
	public List<ArtFieldInfo> getFieldsOnLevel(ArtFieldInfo parent,
		...
		if (level == 0) {
			...
			List<ArtFieldInfo> fields = new ArrayList<ArtFieldInfo>();
			fields.add(new ArtFieldInfo(thisField, new FieldInfoArrayLength(
					this), new ClassInfoPrimitive(int.class)));
			return fields;
		} else {
