...
public class ClassInfoReference extends ClassInfo {
  ...
  public void initialize(
      ClassInfo sc, ClassInfo[] ins, ClassInfo[] inners, FieldInfo[] fields,
      MethodInfo[] methods, ConstructorInfo[] ctors) {
    if (!isInitialized) {
      isInitialized = true;
      this.superClass = sc;
      this.interfaces = ins;
  ...
  public ObjectInfo cloneMockup(Object object, int level) {
    ...
    if (object == null) {
      ...
      for (FieldInfo field : allFields) {
        ...
        ObjectInfo inner = field.getType().cloneMockup(null, level + 1);
        if (inner != null) {
      ...
      for (FieldInfo field : allFields) {
        ...
        ObjectInfo inner = field.getType().cloneMockup(fieldValue, level + 1);
        if (inner != null) {
          innerObjects.add(inner);
        }
      }
  ...
  public List<ArtFieldInfo> getFieldsOnLevel(
    ...
    if (level == 0) {
      ...
      for (FieldInfo fieldInfo : allFields) {
        List<ArtFieldInfo> innerFields = fieldInfo.getType().getFieldsOnLevel(
            thisField, fieldInfo, level - 1);
