...
public class JDartRunner {
	...
	public Pair<List<TestInput>, Integer> runJDart(LearnTestParams learntestParams, String mainClass){
		try {
			...
			JDartProcess jdartCore = new JDartProcess(JDartSettings.getInstance());
	...
	public List<TestInput> runJDartOnDemand(LearnTestParams learntestParams, String mainClass,String jdartInitTc, int node, int branch){
		try {
			...
			JDartProcessOnDemand jdartCore = new JDartProcessOnDemand(JDartSettings.getInstance());
	...
	private JDartParams initJDartParams(LearnTestParams learntestParams) throws CoreException {
		...
		params.setAppProperties(learntestParams.getSystemConfig(LearntestSystemVariable.JDART_APP_PROPRETIES));
		params.setSiteProperties(learntestParams.getSystemConfig(LearntestSystemVariable.JDART_SITE_PROPRETIES));
		params.setOnDemandSiteProperties(learntestParams.getSystemConfig(LearntestSystemVariable.JDART_ON_DEMAND_SITE_PROPERTIES));
