...
public class DecisionLearner implements CategoryCalculator {
	...
	public DecisionLearner(JavailpSelectiveSampling selectiveSampling, CfgConditionManager manager, boolean onlySPF) {
	...
	private Pair<Formula, Formula> learn(BreakpointData bkpData) throws SavException {
		...
		if (manager.isRelevant(bkpData.getLocation().getLineNo())/*!manager.isEnd(bkpData.getLocation().getLineNo())*/) {
