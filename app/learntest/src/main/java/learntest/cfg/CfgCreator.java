...
//	private List<CfgDecisionNode> temporaryContinueNodeList = new ArrayList<CfgDecisionNode>();
	...
	protected CFG convert(IfStmt n) {
		...
		if (hasBreakStmt) {
			...
			if (breakStmtToParentStmt != null) {
				...
				attachExecutionBlock(cfg, decision, elseThen, decision,
						false);
				temporaryBreakNodeList.add(decision);
	...
	protected void dealWithBreakStmt(Object n, CFG cfg){	
		if(temporaryBreakNodeList.size() != 0 &&((Node)n).contains(temporaryBreakNodeList.get(0).getAstNode().getParentNode())){
			for(int i = 0 ; i < temporaryBreakNodeList.size(); i ++){
				...
				CfgBranchEdge branch = newBranchEdge(temporaryBreakNodeList.get(i), cfg.getExit(), temporaryBreakTrueOrFalseList.get(i));
				cfg.addEdge(branch);
	...
	private void attachExecutionBlock(CFG cfg, CfgDecisionNode decision,
		...
		for (CfgEdge bodyEntryOut : thenBlk.getEntryOutEdges()) {
			...
			cfg.addEdge(branch);
			cfgEdgeMap.put(decision.toString() + bodyEntryOut.getDest().toString() + passCond, branch);
		}
	...
	private void attachExecutionBlock(CFG cfg, CfgDecisionNode decision,
		...
		cfg.addEdge(branch);
		cfgEdgeMap.put(decision.toString() + thenSuccessor.toString() + passCond, branch);
