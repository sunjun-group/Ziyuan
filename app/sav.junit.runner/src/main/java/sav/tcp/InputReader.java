...
public class InputReader extends DataInputStream {
	...
	public <T> T readSerializableObj() throws IOException {
	...
	@SuppressWarnings("unchecked")
	public <T>List<T> readSerializableList() throws IOException {
		int size = readVarInt();
		if (size == 0) {
	...
	@SuppressWarnings("unchecked")
	public <K, V>Map<K, V> readSerializableMap() throws IOException {
		int size = readVarInt();
		if (size == 0) {
			return new HashMap<>();
		}
