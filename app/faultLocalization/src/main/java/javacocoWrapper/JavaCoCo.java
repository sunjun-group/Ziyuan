...
public class JavaCoCo implements ICodeCoverage {
	...
	public CoverageReport run(List<String> testingClassNames, List<String> junitClassNames)
		...
		for(String junit: junitClassNames){
			instrumenteds.add(instr.instrument(getTargetClass(junit), junit));
		}
		
		for(int j = 0; j < junitClassNames.size(); j++){
			String testingClassName = junitClassNames.get(j);
		...
		for(int i = 0; i < testcases.size(); i++){
			...
			for(String testingClassName: junitClassNames){
			...
			for (final IClassCoverage cc : coverageBuilder.getClasses()) {
				...
				for (String className : junitClassNames) {
					if (cc.getSourceFileName().equals(
	...
	public CoverageReport run(List<String> testingClassNames, Class<?> junitClass)
		...
		List<Request> testcases = extractTestCasesAsRequests(memoryClassLoader
				.loadClass(junitClass.getName()));
		for(int i = 0; i < testcases.size(); i++){
			Request testcase = testcases.get(i);
