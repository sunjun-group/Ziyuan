...
public class AddFieldCodeVisitor<A> extends ModifierVisitorAdapter<A> {
	...
	public AddFieldCodeVisitor(String methodName, int lineNo, Variable learnVar) {
			...
			for (int i = 0; i < stmts.size(); i++) {
				...
				if (stmt.getBeginLine() == lineNo) {
					if (!learnVar.getType().equals("int")) {
						...
						MethodCallExpr call = new MethodCallExpr(clazz, "fieldToNew");
