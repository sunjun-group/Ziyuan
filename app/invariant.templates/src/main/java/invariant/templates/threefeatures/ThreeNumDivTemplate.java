...
public class ThreeNumDivTemplate extends ThreeFeaturesTemplate {

	public ThreeNumDivTemplate(List<List<ExecValue>> passExecValuesList, List<List<ExecValue>> failExecValuesList) {
	...
	public List<List<Eq<?>>> sampling() {
		...
		if (ev1.getType() == ExecVarType.INTEGER) {
			...
			sample1.add(new Eq<Number>(v3, 1L));
		} else {
