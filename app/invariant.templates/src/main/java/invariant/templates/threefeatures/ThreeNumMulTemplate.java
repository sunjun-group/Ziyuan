...
public class ThreeNumMulTemplate extends ThreeFeaturesTemplate {

	public ThreeNumMulTemplate(List<List<ExecValue>> passExecValuesList, List<List<ExecValue>> failExecValuesList) {
	...
	public boolean checkPassValue(List<ExecValue> evl) {
		...
		// first feature must be equals to multiply between second and third feature
	...
	public boolean checkFailValue(List<ExecValue> evl) {
		// list of pass and fail exec value only has three features
		// first feature must not be equals to multiply between second and third feature
		double v1 = evl.get(0).getDoubleVal();
		double v2 = evl.get(1).getDoubleVal();
		double v3 = evl.get(2).getDoubleVal();
		return v1 != v2 * v3;
