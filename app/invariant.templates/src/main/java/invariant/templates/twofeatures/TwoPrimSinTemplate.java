...
public class TwoPrimSinTemplate extends TwoFeaturesTemplate {

	public TwoPrimSinTemplate(List<List<ExecValue>> passExecValuesList, List<List<ExecValue>> failExecValuesList) {
	...
	public boolean check() {
		...
		// first feature must be equals to sin of second feature
		for (List<ExecValue> evl : passExecValuesList) {
			double v1 = evl.get(0).getDoubleVal();
			double v2 = evl.get(1).getDoubleVal();
			if (v1 != Math.sin(v2)) return false;
		}
				
		// first feature must not be equals to sin of second feature
	...
	public List<List<Eq<?>>> sampling() {
		...
		sample3.add(new Eq<Number>(v1, 0.0));
