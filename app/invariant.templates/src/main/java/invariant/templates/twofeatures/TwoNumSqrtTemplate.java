...
public class TwoNumSqrtTemplate extends TwoFeaturesTemplate {

	public TwoNumSqrtTemplate(List<List<ExecValue>> passExecValuesList, List<List<ExecValue>> failExecValuesList) {
	...
	public List<List<Eq<?>>> sampling() {
		...
		if (ev1.getType() == ExecVarType.INTEGER) {
			sample2.add(new Eq<Number>(v1, -2));
			sample2.add(new Eq<Number>(v2, 4));
		} else if (ev1.getType() == ExecVarType.LONG) {
			sample2.add(new Eq<Number>(v1, -2L));
			sample2.add(new Eq<Number>(v2, 4L));
		} else {
			sample2.add(new Eq<Number>(v1, -2.0));
			sample2.add(new Eq<Number>(v2, 4.0));
		}
